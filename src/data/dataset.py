...
class TransliterationDataset(Dataset):
    ...
    def read_csv_data(self, src_lang, trg_lang):
        ...
        self.src_charset = CharSet(language=src_lang)
        self.trg_charset = CharSet(language=trg_lang)
        
        self.src_words, self.trg_words = corpus_train.iloc[:, 0], corpus_train.iloc[:, 1]
        self.train_dataset = self.build_dataset(corpus_train, self.src_charset, self.trg_charset)
        self.test_dataset = self.build_dataset(corpus_test, self.src_charset, self.trg_charset)
        self.valid_dataset = self.build_dataset(corpus_valid, self.src_charset, self.trg_charset)
    ...
    def build_dataset(self, data, src_charset, trg_charset):        
        ...
        input_tensor = torch.zeros((len(data[0]), MAX_SEQUENCE_LENGTH), dtype=torch.long)
        output_tensor = torch.zeros((len(data[1]), MAX_SEQUENCE_LENGTH), dtype=torch.long)
        
        for i in range(len(data)):
            in_seq = data[0][i].ljust(MAX_SEQUENCE_LENGTH-1, '#') + '$'
            out_seq = data[1][i].ljust(MAX_SEQUENCE_LENGTH-1, '#') + '$'
